...
export default function MiniJobs(): JSX.Element {
    ...
    const toalJobs = jobList.length;
    const appliedJobs = jobList.filter((job) => job.status === 'Applied').length;
    const pendingInterview = jobList.filter((job) => job.status === 'Pending Interview').length;
    const interviewed = jobList.filter((job) => job.status === 'Interviewed').length;
    const waitingForResponse = jobList.filter(
        (job) => job.status === 'Waiting for Response'
    ).length;
    const archivedJobs = jobList.filter((job) => job.status === 'Archived').length;
    const savedJobs = jobList.filter((job) => job.status === 'Saved').length;
    ...
    const totalItems = jobStats.length;
